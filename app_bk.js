/******************************************************************
 Fake News Challenge — auto-load stimuli from filenames like:
 "1-f-ll-3-vl-2.png"
...
function computeInfluence(results){
  const byLing = { high: [], low: [] };
  const byVis  = { high: [], low: [] };

  results.forEach(r=>{
    byLing[r.stim.lingQuality].push(r.correct ? 1 : 0);
    byVis[r.stim.visQuality].push(r.correct ? 1 : 0);
  });

  const accLingHigh = mean(byLing.high);
  const accLingLow  = mean(byLing.low);
  const accVisHigh  = mean(byVis.high);
  const accVisLow   = mean(byVis.low);

  const lingDelta = accLingHigh - accLingLow;
  const visDelta  = accVisHigh - accVisLow;

  const avgRt = mean(results.map(r=>r.rtSec));
  const acc   = mean(results.map(r=>r.correct ? 1 : 0));

  if(acc < 0.55 && avgRt < 8){
    return {
      label: "⚡ HEURISTIC-DRIVEN",
      explanation: "Fast decisions with low accuracy suggest quick heuristics (gut feeling) dominated.",
      metrics: { accLingHigh, accLingLow, accVisHigh, accVisLow, lingDelta, visDelta }
    };
  }

  if(Math.abs(lingDelta - visDelta) < 0.07){
    return {
      label: "⚖️ BALANCED JUDGE",
      explanation: "Your accuracy changed similarly across language and visual presentation.",
      metrics: { accLingHigh, accLingLow, accVisHigh, accVisLow, lingDelta, visDelta }
    };
  }

  if(lingDelta > visDelta){
    return {
      label: "📝 LANGUAGE-DRIVEN JUDGE",
      explanation: "Polished writing influenced you more than layout or design.",
      metrics: { accLingHigh, accLingLow, accVisHigh, accVisLow, lingDelta, visDelta }
    };
  }

  return {
    label: "🎨 VISUAL-DRIVEN JUDGE",
    explanation: "Professional-looking layouts influenced your decisions more than writing style.",
    metrics: { accLingHigh, accLingLow, accVisHigh, accVisLow, lingDelta, visDelta }
  };
}
...
function renderLoading(){
  setHTML(`
    <div class="card">
      <div class="header">
        <div>
          <div class="title">Loading stimuli…</div>
    ...
    <div class="card">
      <div class="header">
        <div>
          ...
          <div class="sub">12 articles. Decide: <b>TRUE</b> or <b>FAKE</b>.</div>
        </div>
        <div class="pill">Stimuli: <b>${State.ALL_STIMULI.length}</b> • Stories: <b>${storySet.size}</b></div>
      </div>
      <div class="content">
        <div class="row">
          <div class="col">
      ...
      <div class="content">
        <div class="row">
          <div class="col">
            ...
            <div class="small">
              Average player today: <b>${avgPlayerCorrect ? avgPlayerCorrect.toFixed(1) : "—"} / ${total}</b><br/>
              ${percentile === null
                ? `You’re the first player on this device — play again to build comparisons!`
                : `You did better than <b>${percentile}%</b> of players on this device.`}
            </div>

            <div class="btnbar">
              <button onclick="State.screen='intro'; render();">Finish</button>
              <button class="primary" onclick="State.screen='result2'; render();">Next ▶</button>
            </div>
          </div>
          ...
          <div class="col">
            <div class="box">
              ...
              <div class="small">
                ...
                Next: you’ll see how you compare on a speed–accuracy map.
              </div>
            </div>
          </div>
        </div>
...
function renderResult2(){
  ...
  setHTML(`
    <div class="card">
      <div class="header">
        <div>
          <div class="title">⏱️ SPEED vs ACCURACY</div>
          <div class="sub">Your dot vs other players on this device</div>
        </div>
        <div class="pill">You: <b>${Math.round(last.accuracy*100)}%</b> • <b>${formatSeconds(last.avgRt)}</b></div>
      </div>
      ...
      <div class="content">
        <div class="grid2">
          <div class="box">
            <h3>Map</h3>
            <canvas id="scatter" width="900" height="520"></canvas>
            <div class="note">Higher is better accuracy. Left is faster.</div>
          </div>
          <div class="box">
            <h3>Your category</h3>
            <div class="val" id="perfCat">—</div>
            <div class="small" id="perfText">—</div>
            <div class="hr"></div>
            <div class="small">
              People often do better when they take a little more time — but speed can be fine if you’re consistent.
            </div>
          </div>
        </div>

        <div class="btnbar">
          <button onclick="State.screen='result1'; render();">◀ Back</button>
          <button class="primary" onclick="State.screen='result3'; render();">Next ▶</button>
        </div>
      </div>
    </div>
  `);
...
function performanceCategory(run){
  ...
  if(acc >= 0.75)           return { title:"CAREFUL & ACCURATE", text:"High accuracy—taking time seems to help you." };
  ...
  if(t <= 10)               return { title:"FAST & OK", text:"Good pace—accuracy is decent." };
...
function drawScatter(runs, highlightIndex){
  ...
  const padL=48, padR=18, padT=18, padB=42;

  const times = runs.map(r=>r.avgRt);
  const accs  = runs.map(r=>r.accuracy);

  const tMin = 0;
  const tMax = Math.max(20, ...times, 25);
  const aMin = 0.4;
  const aMax = 1.0;

  function x(t){ return padL + (1 - (t - tMin)/(tMax - tMin)) * (w - padL - padR); }
  function y(a){ return padT + (1 - (a - aMin)/(aMax - aMin)) * (h - padT - padB); }
  ...
  ctx.fillText("Accuracy ↑", padL, 14);
  ctx.fillText("Time → (left is faster)", w - 190, h - 12);

  runs.forEach((r, i)=>{
    const px = x(r.avgRt);
    const py = y(r.accuracy);
...
function renderResult3(){
  ...
  const lingHighCorrect = Math.round(m.accLingHigh * lingHighTotal);
  const visHighCorrect  = Math.round(m.accVisHigh  * visHighTotal);

  setHTML(`
    <div class="card">
      <div class="header">
        <div>
          <div class="title">🧠 WHAT INFLUENCED YOUR JUDGEMENTS?</div>
        ...
        <div class="grid2">
          <div class="box">
            ...
            <div class="small">
              ...
              ✔ Correct <b>${lingHighCorrect} / ${lingHighTotal}</b> times
              <div class="hr"></div>
              When <b>VISUAL DESIGN</b> looked professional:<br/>
              ✔ Correct <b>${visHighCorrect} / ${visHighTotal}</b> times
            </div>
          </div>
          ...
          <div class="box">
            ...
            <div class="hr"></div>
            <div class="note">
              (This is inferred from how your accuracy changed when language/visual quality changed.)
            </div>
          </div>
        </div>

        <div class="btnbar">
          <button onclick="State.screen='result2'; render();">◀ Back</button>
          <button class="primary" onclick="State.screen='result4'; render();">Next ▶</button>
        </div>
      </div>
    </div>
  `);
